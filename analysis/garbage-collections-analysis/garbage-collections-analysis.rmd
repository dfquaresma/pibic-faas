...
#### Experimento
- Execução de 10.000 requisições.
- Execução sequencial das chamadas à função.

#### Função 
- Linguagem: java8
- Lógica de negócio: Redimencionamento de uma imagem de 1920x1080 pixels para 192x108 pixels. 
...
qNoCollect = quantile(nocollect$business_time.ms., c(.0, .25, .50, .75, .90, .99, .999))
qNoCollect
```
...
qWithCollect = quantile(withcollect$business_time.ms., c(.0, .25, .50, .75, .90, .99, .999))
qWithCollect
```

#### Comparação
```{r}
qWithCollect / qNoCollect
```
